...
# run just one test
if [[ $specific != "" ]]; then
    run_and_check $testdir $specific $contrunning $verbose $cmperr 1
    exit 0
fi
...
(( testnum = 1 ))
while true; do
    run_and_check $testdir $testnum $contrunning $verbose $cmperr 0
    (( testnum = $testnum + 1 ))
done
